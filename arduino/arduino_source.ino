...
#include < LiquidCrystal.h > #include < Servo.h > Servo servo;
...
void loop(void) {
    ...
    while (mySerial.available()) { // output to the serial window
    ...
    if (index == -1) {
        ...
        } else if (response[index + 7] == '1') { // 모터 동작
